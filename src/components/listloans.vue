    ...
    <div>
        <div>
            <v-data-table :headers="headers" :items="indexSn" class="elevation-1">
                <template v-slot:top>
                    <v-toolbar flat>
                        <v-toolbar-title>Loan Applicant</v-toolbar-title>
...
async listContributors(){
    try{
   const response = await this.$http.get('/list-loans', )
   this.desserts = response.data.list_contributors_loan;
   this.desserts = [];
   //Getting the data and pushing it into the desserts array for each object in the array 
   response.data.list_contributors_loan.forEach(el => {
