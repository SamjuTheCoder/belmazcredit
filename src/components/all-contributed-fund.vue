    ...
    <div>
        <div>
            <v-data-table :headers="headers" :items="indexSn" class="elevation-1">
                <template v-slot:top>
                    <v-toolbar flat>
                        <v-toolbar-title>All Contributions</v-toolbar-title>
               ...
               <template v-slot:item.contribution_amount="{item}">
                ...
                <template v-slot:item.action="{item }">
                    <div>
                        <v-btn style="color:green" v-if="item.status == 1" @click="toogleStatus(item.uid)" text >Payment Confirmed</v-btn>
                        <v-btn style="color:red" text v-else @click="toogleStatus(item.uid)" >Confirm Payment</v-btn>
                    </div>
                </template>
...
export default {
 ...
 beforeMount(){
    const list = this.listContributions(); 
 ...
 async toogleStatus(uid){
            try {
                const url = "/confirm-contribution-payment";
...
async listContributions(){
    try{
   const response = await this.$http.get('/all-contributions', )
   this.desserts = response.data.contributors;
   this.desserts = [];
   //Getting the data and pushing it into the desserts array for each object in the array 
   response.data.contributors.forEach(el => {
