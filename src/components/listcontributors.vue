    ...
    <div>
        <div>
            <v-data-table :headers="headers" :items="indexSn" class="elevation-1">
                <template v-slot:top>
                    <v-toolbar flat>
                        <v-toolbar-title>List of Contributors</v-toolbar-title>
                ...
                <template v-slot:item.sponsorid="{item}">
                <div>
                    {{ item.sponsorid }}
                </div>
                </template>
                ...
                <div v-if="item.phaseid == 1">
                    {{ formatMoney(10000) }}
                </div>
                 <div v-else-if="item.phaseid == 2">
                    {{ formatMoney(50000) }}
                </div>
                 <div v-else-if="item.phaseid == 3">
                    {{ formatMoney(300000) }}
                </div>
                 <div v-else-if="item.phaseid == 4">
                    {{ formatMoney(1500000) }}
                </div>
                </template>
                ...
                <template v-slot:item.action="{item }">
                    <div>
                        <v-btn style="color:green" v-if="item.status == 1" @click="toogleStatus(item.uid)" text >User Activated</v-btn>
                        <v-btn style="color:red" text v-else @click="toogleStatus(item.uid)" >Activate</v-btn>
                    </div>
                </template>
...
export default {
 ...
 async toogleStatus(uid){
            try {
                const url = "/confirm-user-payment";
...
async listContributors(){
    try{
   const response = await this.$http.get('/list-contributors', )
   this.desserts = response.data.list_contributors;
   this.desserts = [];
   //Getting the data and pushing it into the desserts array for each object in the array 
   response.data.list_contributors.forEach(el => {
            ...
            this.desserts.push(
                {
                    ...
                    sponsorid: el.sponsor_id,
                    phaseid: el.pid,
                    accountname: el.bankname,
