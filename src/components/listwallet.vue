        ...
        <div>
            <v-data-table :headers="headers" :items="indexSn" class="elevation-1">
                ...
                <div>
                 ...
                 <template v-slot:item.walletamount="{item}">
                <div>
                    {{ formatMoney(item.walletamount) }}
                </div>
                </template>
...
async listContributors(){
    try{
   const response = await this.$http.get('/list-wallets', )
   ...
   response.data.list_wallets.forEach(el => {
